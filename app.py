...
def get_db_connection():
    ...
    try:
        # Verifica se a nova secret (DATABASE_URL) existe
        if "DATABASE_URL" not in st.secrets:
            st.error("❌ Secrets not configured! Please add your DATABASE_URL.")
            return None
            
        # O psycopg2 conecta perfeitamente usando a URL completa direta
...
def main():
    ...
    if "DATABASE_URL" not in st.secrets:
        ...
        st.code('DATABASE_URL = "postgresql://postgres:SUA_SENHA_AQUI@db.jcvczxifkgldjledqjyk.supabase.co:5432/postgres"', language="toml")
        return
